$(document).ready(function() {
    var options = {
        ...
        init:'sdfasdfafa',
        plugins: 'xhtml',
        toolbar: "bold,italic,underline,strike,color,size|orderedlist,bulletlist,table|superscript,subscript,pastetext|link,image",
        ...
        bbcodeTrim: true,
        style: 'assets/sceditor/minified/jquery.sceditor.default.min.css',
        autoUpdate: true,
        width: 880,
        }
    var initEditor = function() {
        $("#question").sceditor(options);
    };
...
$(function(){
                  ...
                  success:function(data){
                    ...
                    var problemObj=s.data;
                      ...
                      if (problemObj!= undefined && problemObj != null && problemObj != "false") {			
                        ...
                        $("#question").sceditor('instance').val(app.problem.title)
                      }
                  },
                });
    ...
    $("#savelink").click(function(e){
        ...
        if($("#question").sceditor('instance').val()==""){
            alert("题干不能为空")
            return false
        }
       ...
       if(app.problem.qtype=="1"||app.problem.qtype=="2"||app.problem.qtype=="6"){
        if(app.problem.qtype=="1"){
           ...
           var m=0;
            for(var i=0;i<app.fill.length;i++){
                if(app.fill[i].title.trim()==undefined||app.fill[i].title.trim()==""){
                    m++
                }
            }
            if(m!=0){
        ...
        var s=false;
            ...
            for(var i=0;i<app.Key.length;i++){
                if(app.Key[i].title.trim()==undefined||app.Key[i].title.trim()==""){
                    m++
                }
            }
        ...
        if(app.problem.rightAnswer.trim()!=""){
            s=true
        }
       ...
       var labels=$("#taginput").val()
        ...
        //转成他需要的格式
         ...
         var a={}
         app.problem.title= $("#question").sceditor('instance').val()
